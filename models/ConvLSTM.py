...
class CLSTM_cell(nn.Module):
    ...
    """
    def __init__(self, shape, input_chans, filter_size, num_features, use_bias):
...
class CLSTM(nn.Module):
    ...
    """
    def __init__(self, shape, input_chans, filter_size, num_features,num_layers,use_bias=False):
        ...
        cell_list=[]
        cell_list.append(CLSTM_cell(self.shape, self.input_chans, self.filter_size, self.num_features,use_bias).cuda())#the first
        #one has a different number of input channels

        for idcell in xrange(1,self.num_layers):
            cell_list.append(CLSTM_cell(self.shape, self.num_features, self.filter_size, self.num_features,use_bias).cuda())
...
class ResCLSTM_cell(nn.Module):
    ...
    """
    def __init__(self, shape, input_chans, filter_size, num_features, use_bias):
        super(ResCLSTM_cell, self).__init__()
...
class ResCLSTM(nn.Module):
    ...
    def __init__(self, shape, input_chans, filter_size, num_features,num_layers,use_bias=False):
        ...
        self.num_layers=num_layers
        cell_list=[]
        cell_list.append(ResCLSTM_cell(self.shape, self.input_chans, self.filter_size, self.num_features,use_bias).cuda())#the first
        #one has a different number of input channels

        for idcell in xrange(1,self.num_layers):
            cell_list.append(ResCLSTM_cell(self.shape, self.num_features, self.filter_size, self.num_features,use_bias).cuda())
        self.cell_list=nn.ModuleList(cell_list)
...
conv_lstm=CLSTM(shape, inp_chans, filter_size, num_features,nlayers,True)
