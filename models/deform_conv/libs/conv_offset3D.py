...
class ConvOffset3D(nn.Conv3d):
    ...
    def __init__(self, filters, **kwargs):
        ...
        **kwargs:
            Pass to superclass. See Conv3d layer in pytorch
        """
        self.filters = filters
        self._grid_param = None
        super(ConvOffset3D, self).__init__(self.filters, self.filters * 2, 3, padding=1, bias=False, **kwargs)
    ...
    def forward(self, x):
        ...
        # (b*c, d, h, w, 2)
        offsets = self._to_bc_d_h_w_2(offsets, x_shape)

        # (b*c, d, h, w)
        x = self._to_bc_d_h_w(x, x_shape)

        # X_offset: (b*c, d*h*w)    grid:(b*c, d*h*w, 2)
    ...
    def _get_grid(self, x):
        batch_size, input_size = x.size(0), (x.size(1), x.size(2), x.size(3))
    ...
    @staticmethod
    def _to_bc_d_h_w_2(x, x_shape):
        """(b, 2c, d, h, w) -> (b*c, d, h, w, 2)"""
        x = x.contiguous().view(-1, int(x_shape[2]), int(x_shape[3]), int(x_shape[4]), 2)
        return x
...
def th_gather_3d(input, coords):
    inds = coords[:, 0] * input.size(1) + coords[:, 1] * input.size(2) + coords[:, 2]
...
def th_map_coordinates(input, coords, order=1):
    ...
    Parameters
    ----------
    input : tf.Tensor. shape = (d, h, w)
    coords : tf.Tensor. shape = (n_points, 3)
    """
