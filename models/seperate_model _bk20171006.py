...
class SeperateModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netMP = networks.motion_P(shape, seq_len, opt.ngf*2,
                                            opt.ngf*2, opt.ngf, opt.norm, not opt.no_dropout, self.gpu_ids)
        ...
        if self.isTrain:
            ...
            '''
            self.optimizer_CE = torch.optim.RMSprop(self.netCE.parameters(), lr = opt.lr, alpha = 0.95)
            self.optimizer_ME = torch.optim.RMSprop(self.netME.parameters(), lr = opt.lr, alpha = 0.95)
            self.optimizer_MP = torch.optim.RMSprop(self.netMP.parameters(), lr = opt.lr, alpha = 0.95)
            self.optimizer_G = torch.optim.RMSprop(self.netG.parameters(), lr = opt.lr, alpha = 0.95)
            self.optimizer_D = torch.optim.RMSprop(self.netD.parameters(), lr = opt.lr, alpha = 0.95)
            '''
    ...
    def backward_G(self):
        ...
        #[self.netCE.forward(real) for real in self.real_X] # [bs,1, oc, s0, s1]*len(self.real_X)
        self.content_codes = torch.split(self.netCE.forward(self.realX),batch_size,dim=0)
        hidden_state_SE = self.netSE.init_hidden(batch_size)
        hidden_state_SE, self.content_code = self.netSE.forward(torch.cat(self.content_codes,1),hidden_state_SE)
        ...
        ####[self.netME.forward(real) for real in self.real_X] # [bs,1, oc, s0, s1]*len(self.real_X)
        #hidden_state_G = self.netG.init_hidden(batch_size*self.pre_len)
        ...
        ###latent_codes = [torch.cat([self.content_code,self.pre_feats[-1].squeeze(1)],1)]
        latent_codes = [torch.cat([self.content_code,self.motion_codes_y[0].squeeze(1)],1)]
        for t in xrange (1, self.pre_len):
            hidden_state_M, pre_feat = self.netMP.forward(self.pre_feats[-1],hidden_state_M)
            self.pre_feats += [pre_feat]
            ###latent_codes += [torch.cat([self.content_code,self.pre_feats[-1].squeeze(1)],1)]
            latent_codes += [torch.cat([self.content_code,self.motion_codes_y[t].squeeze(1)],1)]
        ...
        self.latent_codes = [torch.cat(concat_codes,1)]
        ...
        for t in xrange(1, self.pre_len):
            ...
            for l in xrange(len(self.content_codes)):
                ...
                concat_codes += [torch.cat([self.content_codes[l], self.pre_feats[-1]-self.motion_codes[l]],2)]
            self.latent_codes += [torch.cat(concat_codes, 1)]
            #print 'time step ',l,': ', self.latent_codes[-1].size()
        self.latent_code = torch.cat(self.latent_codes,0)
        hidden_state_G_0, fakes = self.netG.forward(self.latent_code, hidden_state_G)
        '''
        ...
        self.loss_G = self.loss_pix + self.loss_gan + self.loss_dif
